...
The app is installable as a PWA: `public/manifest.webmanifest` (standalone display, #0B0A08 theme) plus PNG icons rendered from the favicon mark (`apple-touch-icon.png` 180, `icon-192.png`, `icon-512.png`, `icon-512-maskable.png`). `public/service-worker.js` precaches the shell (`/`, styles, fonts, favicon, manifest) and serves navigations network-first with cache fallback, `/assets/*` + `/styles.css` stale-while-revalidate, and `/fonts/*` cache-first, so visited pages keep working offline. Cache invalidation is per-deploy: the committed worker carries a `__BUILD_VERSION__` placeholder (falls back to `dev` when unstamped) that `scripts/stamp-service-worker.mjs` rewrites in place with `VERCEL_GIT_COMMIT_SHA` during the Vercel build — every deploy byte-changes the worker, which installs, `skipWaiting`s, claims clients, and drops all previous-version caches on activate. `vercel.json` also serves `/service-worker.js` with `Cache-Control: no-cache`. An inline script in `app/ui/document.tsx` registers it with `{ updateViaCache: 'none' }` and reloads the page once on `controllerchange` (only when a previous controller existed, so first installs never reload) — users pick up new code on the very visit that fetched it. Registration is skipped on `localhost`/`127.0.0.1` so dev is never affected by SW caching.
