...
The app is installable as a PWA: `public/manifest.webmanifest` (standalone display, #0B0A08 theme) plus PNG icons rendered from the favicon mark (`apple-touch-icon.png` 180, `icon-192.png`, `icon-512.png`, `icon-512-maskable.png`). The installed iOS app runs standalone with `black-translucent` status bar and `viewport-fit=cover`, so the viewport extends under the status bar: the AppShell header compensates with `pt-[env(safe-area-inset-top)]` (the chrome backdrop stays under the notch) and the BottomBar pads with `safe-area-inset-bottom`. iOS launch screens are `apple-touch-startup-image` PNGs in `public/splash/` — the brand mark centered on #0B0A08, one per common iPhone portrait size — matched by device-width/height/pixel-ratio media queries in `app/ui/document.tsx`; they are fetched by the OS at install time, so the service worker does not precache them. Android derives its splash from the manifest's `background_color` + 512 icon. `public/service-worker.js` precaches the shell (`/`, styles, fonts, favicon, manifest) and serves navigations network-first with cache fallback, `/assets/*` + `/styles.css` stale-while-revalidate, and `/fonts/*` cache-first, so visited pages keep working offline. Unvisited pages are warmed proactively: `app/assets/entry.ts` calls `warmOfflineCache` (`app/assets/offline.ts`) after boot and after every document change (debounced 2s), posting `{ type: 'warm-routes', urls }` to the worker — the static pages plus every screen of every active trip, with expense-edit pages after those, capped at 200 URLs — and the worker fetches each same-origin URL and puts it in the current cache, so offline navigation reaches per-trip pages the user never opened. The same debounced pass also warms the JS module graph: `warmScreenModules` dynamically `import()`s every `*-screen.tsx` asset URL once per page load, so the browser pulls each screen's full transitive import graph (business, framework, node_modules) through the worker's `/assets/*` handler — unvisited screens hydrate and mutate offline, not just render their shell. Cache invalidation is per-deploy: the committed worker carries a `__BUILD_VERSION__` placeholder (falls back to `dev` when unstamped) that `scripts/stamp-service-worker.mjs` rewrites in place with `VERCEL_GIT_COMMIT_SHA` during the Vercel build — every deploy byte-changes the worker, which installs, `skipWaiting`s, claims clients, and drops all previous-version caches on activate. `vercel.json` also serves `/service-worker.js` with `Cache-Control: no-cache`. An inline script in `app/ui/document.tsx` registers it with `{ updateViaCache: 'none' }` and reloads the page once on `controllerchange` (only when a previous controller existed, so first installs never reload) — users pick up new code on the very visit that fetched it. Registration is skipped on `localhost`/`127.0.0.1` so dev is never affected by SW caching.
