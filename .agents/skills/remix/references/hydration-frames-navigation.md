...
Because they render on the server first, a `clientEntry` component's initial render must be deterministic across server and client. Never branch initial state on a browser-only probe in the component body (`typeof VideoDecoder`, `'gpu' in navigator`, `window.matchMedia`) — the server takes one branch, the hydrating client takes the other, and the mismatch duplicates DOM. Initialize to the same state both sides can render, then detect after mount and re-render:
...
return () => (
  <div mix={ref(() => queueMicrotask(checkSupport))}>
    {unsupported ? <p>…</p> : <input … />}
  </div>
)
```
