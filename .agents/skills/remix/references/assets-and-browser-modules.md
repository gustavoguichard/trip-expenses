...
## Packages the Compiler Cannot Serve

Some npm packages compile fine in bundlers but fail through the asset server with `IMPORT_RESOLUTION_FAILED`, because the compiler resolves every import specifier — including Node-guarded ones a browser would never execute. Two shapes trigger it:

- A package mapping a module to `false` via the `package.json` `browser` field (the compiler does not honor that convention).
- Emscripten/WASM glue with environment-guarded imports like `await import("module")`.

For such packages, bypass the compiler: serve the package's **prebuilt browser bundle raw** through an app route with an allowlist of filename → `node_modules` dist path (see the `vendor` route in `app/actions/controller.tsx`, which serves mediabunny and ONNX Runtime Web this way — ORT's runtime also fetches its `.mjs`/`.wasm` loader files from the same route via `env.wasm.wasmPaths`). In browser code, keep typing with `import type` (erased at compile) and load at runtime with a dynamic `import(url)` whose specifier is a **variable**, so the compiler leaves it unresolved:

```ts
import type * as Mediabunny from 'mediabunny'

function loadVendorModule<T>(file: string) {
  const url = `/vendor/${file}`
  return import(url) as Promise<T>
}
export const loadMediabunny = () =>
  loadVendorModule<typeof Mediabunny>('mediabunny.mjs')
```

Two probing quirks while debugging asset URLs: the asset server answers GET only (a `curl -I` HEAD probe 404s even for URLs that serve fine), and in pnpm monorepos the rewritten import URLs point into percent-encoded `node_modules/.pnpm/...` paths — copy them from a compiled module's output rather than constructing them by hand.
