...
**Parallel agents each pass their own `--session`, named after their lane or task.** Without it they all share one default session: one agent's `close --all` kills another's login, and their navigations bleed into each other, so a screenshot captures a sibling's page. Close your own session by name when done — never `close --all` while another agent may be running.
...
Pass captures an **absolute path into the scratchpad**. A relative path resolves against the daemon's working directory, not yours, so the file lands wherever the daemon happened to start. The daemon can also drop an `auth.json` into that same directory — when it is the repo root, `pnpm run lint` then fails on a file nobody wrote.
...
- **Never trust a click's own success.** Both `click @ref` and `find <role> click` report done while landing nothing. A center-click on an element outside the viewport lands on whatever occupies that spot instead — dismissing a dialog rather than pressing its button — so `scrollintoview` the target first. Anything that moves the page between the snapshot and the click (an autoplaying video, a late-loading image) misses the same way, and some pages miss every single time. Drive those clicks through `eval` with `.click()` on the element and assert the DOM change you expect; a click that produces no DOM or network change is a suspected miss, not an app bug.
- **Match text on the casing in the DOM, not on screen.** A CSS `text-transform: uppercase` changes what a snapshot shows but not `textContent`, so a name-based locator or an assertion written from the screenshot finds nothing.
- **A combobox that commits on `pointerdown` never selects via a synthetic click.** The option is gone before the click lands. Type into the input with `fill`, then `press Enter`.
- **`upload` hands the page a File that `createImageBitmap` may refuse to decode.** The flow reports a file chosen and then fails inside its own decode step, which reads as an app bug. When the surface decodes images client-side, serve the fixture over HTTP and build the File in-page with `eval` instead.
