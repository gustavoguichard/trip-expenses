...
- A **seven-lane product evolution** (schema + business slices, UI slices, a Python pipeline slice, then a fix pass and a polish pass) ran healthy single agents throughout at **~165–330k (17–33%)** each, with one outlier: an engine + players slice reached **~363k (36%)** because it verified itself in-browser against real data — the same seam as the "split the in-browser smoke out" lesson above, and the same remedy. The **5-finder single-vote review** over the merged delta (Sonnet, one dimension each, **~535k** total, no verifier stage) found **1 critical and 6 majors after every unit, lint and type gate was already green**. The critical was a composition bug between two features built in separate lanes — single-pose matching against stage scoring — which no builder had a lens for, since per-lane gates never exercise the seam between lanes. The authorization dimension returned **zero findings across ~40 commits of new query surface**: that is what charters that state the tenancy pattern explicitly buy you. Both follow-up passes were comfortable single agents — 12 fixes at **~201k**, 5 polish items at **~165k**.
